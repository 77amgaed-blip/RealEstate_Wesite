...
if (!isset($_SESSION['last_insert_token'])) {
    $_SESSION['last_insert_token'] = '';
} 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($_POST['insert_token'] !== $_SESSION['last_insert_token']) {
     ...
     @$category_project=$_POST['category_project'];
       @$addproject=$_POST['addproject'];

   @$db_nmae_images=array();//مصفوفة سوف نخزن فيها اسما الصور معا المسارات في قاعدة البيانات

       //imag start file
     @$img_project=$_FILES['img_project'];
     $image_temp = $img_project['tmp_name'];
     $image_size =$img_project['size'];
     $allwoed_extendions = array('jpg','gif','jpeg','png');

     $max_size=5 * 1024 * 1024 ; 

        @$image_rxtensions =strtolower(end(explode('.',$img_project['name'] )));
     //end imag file
     if(isset( $addproject)){//start 

     if(isset($project_name) && isset($desc_project) && isset($category_project) &&isset($project_name)&& $img_project['error'] === UPLOAD_ERR_OK ):
        
       
         

                //فحص الحجم        
               if(   $image_size > $max_size ){//التاكد من حجم الملف
        //    echo "<script>alert('$image_rxtensions');</script>"; 

                 $error[] ='<div>حجم الملف اكبر من 5ميجا</div>';
                 } 

                //فحص نوع الملف
                  if(! in_array($image_rxtensions,$allwoed_extendions))://فحص امتداد الملف هل موجود داخل الدالمصفوف
            
                    $error[] ='<div>خطأ فى امتداد الملف</div>';
                  endif;


                else:
                    $error[] ='<div>خطأ أدخل البيانات بشكل صحيح </div>';

     endif;

       if(empty($error)){
         $upload_dir = 'images/projects/';//معالجة رفع الصورة
        if (!file_exists($upload_dir)) {//تفحص اذا كان الملف موجود والاتنشءه
            mkdir($upload_dir, 0755, true);
        }

        @$new_filename_imag = rand(0,50000) .'.'.$image_rxtensions;//الاسم الجديد لصورة 
        $new_img_project = $upload_dir . $new_filename_imag;
         if(move_uploaded_file( $image_temp ,  $new_img_project )){//نقل الصورة من موقعها المؤقت إلى المجلد المحدد
             $db_nmae_images[]  = $new_img_project;
               $imag_file=implode(',', $db_nmae_images);//تحويل المصفوفة الى نص اي عناصر المصفوفة
            
        $query="INSERT INTO add_project(project_name,desc_project,category_project,img_project)
        VALUES('$project_name','$desc_project','$category_project','$imag_file')";
